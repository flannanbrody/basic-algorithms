 ...
 */
public class CircusTowerPeople {
	...
	public static void calcPeopleTower(Person[] P) {
		...
		for (int i = 1; i < len; i++) {
			for (int j = i - 1; j >= 0; j--) {
				if (((DP[j] + 1) > DP[i])
						&& (persons[j].height <= persons[i].height)) {
					DP[i] = DP[j] + 1;
					prev[i] = j;
				}
			}
	...
	public static void printPeopleTower(int[] prev, int n) {
		...
		System.out.println("H:" + persons[n].height + " :: W:"
				+ persons[n].weight);
	...
	public static void printPersons(Person[] persons) {
		...
		for (int i = 0; i < len; i++) {
			System.out.println("H:" + persons[i].height + " :: W:"
					+ persons[i].weight);
		}
	}
	...
	public static void readInput() {
		...
		try {
			...
			while (n > 0) {
				...
				persons[i] = new Person(Integer.parseInt(temp[0]),
