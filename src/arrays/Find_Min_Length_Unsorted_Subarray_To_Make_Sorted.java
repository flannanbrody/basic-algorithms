...
public class Find_Min_Length_Unsorted_Subarray_To_Make_Sorted {
	 ...
	 * sorted.
	 * 
	 * Examples: 1) If the input array is [10, 12, 20, 30, 25, 40, 32, 31, 35,
	 * 50, 60], your program should be able to find that the subarray lies
	 ...
	 * a) Scan from left to right and find the first element which is greater than the
	 *    next element.Let s be the index of such an element. In the above example 1, s
	 *    is 3 (index of 30). 
	 * b) Scan from right to left and find the first element (first in right to left order) 
	 *    which is smaller than the next element (next in right to left order). Let e be the 
	 ...
	 *   c) Find the last element (if there is any) in arr[e+1..n-1] which is smaller than
	 *      max, change e to index of this element. In the above example 1, e is changed to 8 
	 *      (index of 35)
	 */
	...
	public static void printUnsorted(int arr[]) {
		...
		System.out.printf(" sorted lies between the indees %d and %d", s, e);
		return;
	}
